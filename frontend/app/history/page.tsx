  ...
  if (isLoading) {
    return (
      <div className="container-wide py-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">추천 히스토리</h1>
    ...
    return (
      <div className="container-wide py-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">아직 추천 기록이 없습니다</h3>
      ...
      <div className="space-y-4">
        {historyData.items.map((item: HistoryListItem) => (
          <Link
            ...
            className="block bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-sm transition-all no-underline"
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                  {item.productName}
                  {item.modelName && <span className="text-gray-600 dark:text-gray-400"> ({item.modelName})</span>}
                </h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{formatDate(item.createdAt)}</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-bold text-primary-600 dark:text-primary-400">
                  {formatPrice(item.recommendedPrice)}
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-400">추천가격</p>
              </div>
            </div>
          </Link>
        ))}
      ...
      {historyData.totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-8">
          <button
            ...
            className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 dark:hover:bg-gray-700 dark:text-gray-300"
          >
            이전
          </button>
