  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">시세 신뢰도</h3>
        <span
      ...
      <div className="relative h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden mb-4">
        ...
        />
        {/* 구간 표시 */}
        <div className="absolute left-1/3 top-0 h-full w-px bg-gray-300 dark:bg-gray-600" />
        <div className="absolute left-2/3 top-0 h-full w-px bg-gray-300 dark:bg-gray-600" />
      </div>

      {/* 상세 정보 */}
      <div className="grid grid-cols-2 gap-4 mb-3">
        <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div className="text-2xl font-bold text-gray-900 dark:text-white">{sampleCount}</div>
          <div className="text-sm text-gray-500 dark:text-gray-400">분석 매물 수</div>
        </div>
        {cvPercent !== null && (
          <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{cvPercent}%</div>
            <div className="text-sm text-gray-500 dark:text-gray-400">가격 편차</div>
          </div>
        )}
        {cvPercent === null && (
          <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className={`text-2xl font-bold ${info.color}`}>{info.label}</div>
            <div className="text-sm text-gray-500 dark:text-gray-400">신뢰 수준</div>
          </div>
        )}
      </div>
